                ...
                for node in nodes:
                    if hasattr(node, 'position'):
                        ...
                        if position != (0,0,0):
                            ...
                            print(node.params['wlan'])
                            print(node.params['position'])
                            print(node.params['associatedTo'])
                            print(node.params['rssi'])
                            print(node.params['apsInRange'])
                            print('------------------------------------')
                            #print(ap1.params['stationsInRange'])
                            #print(ap2.params['stationsInRange'])
                            #print(ap3.params['stationsInRange'])
                            print(node.currentTime)
                        if len(node.position) == 0:
                            nodes.remove(node)
                        mobility.configLinks()
                plot.pause()
...
class replayingMobility2(object):
    'Replaying Mobility Traces'
    timestamp = False
