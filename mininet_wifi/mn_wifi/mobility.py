...
class mobility(object):
    ...
    @classmethod
    def set_pos(self, node, pos, nodes_stations):
        node.params['position'] = pos
        print("-------------------*********************set_pos_117-----------------------****************")
        if mobility.wmediumd_mode == 3 and mobility.thread_._keep_alive:
            node.set_pos_wmediumd(pos)
        hist = node.params.get("hist", "null")
        #guardar historial de posiciones
        rss = node.params.get("rssi", "null")
        ap_connected = node.params.get("associatedTo", "null")
        print("2222222222222222222222222222222222222222222222222222222222222222")
        print(node.name)
        print(node.params)
        f_stations_info= open("/home/mininet/Escritorio/scripts/output/f_stations_info.txt","a+")
        if ap_connected[0] == "":
            #f_stations_info.write('{},{},{},{},{},{}\n'.format("station", "associatedTo","time","battery", "rssi", "position"))
            f_stations_info.write('{},{},{},{},{}\n'.format( str(node.name), str(" "), str(node.params['battery']), str(" "), str(node.params['position']) ))
            #f_stations_info.write("{},{},{},{},{},{},{}\n".format("station","aps", "ap", "rssi", "num_sta", "dist", "cons_bat", "associatedTo"))
        else:
            ap_temp_dis = node.get_distance_to(ap_connected[0]) # distancia del adispositivo al AP
            ap_temp_rssi = node.get_rssi(ap_connected[0],0,ap_temp_dis)  #RSSI del dispositivo al AP
            #f_stations_info.write('{},{},{},{},{}\n'.format("station", "associatedTo","battery", "rssi", "position"))
            f_stations_info.write('{},{},{},{},{}\n'.format( str(node.name), str(ap_connected[0].name), str(node.params['battery']), str(ap_temp_rssi), str(node.params['position']) ))
        #ap_temp_dis = node.get_distance_to(ap_connected) # distancia del adispositivo al AP
        #ap_temp_rssi = node.get_rssi(ap_connected,0,ap_temp_dis)  #RSSI del dispositivo al AP
        #f_stations_info= open("/home/mininet/Escritorio/scripts/output/f_stations_info.txt","a+")
        #f_stations_info.write('{},{},{},{},{},{}\n'.format("station", "associatedTo","time","battery", "rssi", "position"))
        #f_stations_info.write('{},{}\n'.format( str(node.name), str(ap_connected)))
        #f_stations_info.write("{},{},{},{},{},{},{}\n".format("station","aps", "ap", "rssi", "num_sta", "dist", "cons_bat", "associatedTo"))
        #f_stations_info.close()
        
        print("node.py - position to save " + str(node.params['position']))
        del hist[0][0]
        hist[0].append(str(node.params['position']))
        
        if hist != "null":
            print("node.py - con hist")
            var = node.params.get("associatedTo", "null")[0]
            print var
            print(str(type(var)))
            if type(var) == str:
                del hist[2][0]
                ...
                hist[2].append("")
                print(hist)
                print("node.py - es " + str(type(var)))
            else:
                del hist[2][0]
                print(hist)
                hist[2].append(var.name)
                print(hist)
                print("node.py - NO, es " + str(type(var)))
        else:
            print("sin hist")
            
        id_group = node.params.get("id_group", "null")
        
        if id_group == "":
            #----------------------SACAR ESTACIONES QUE ESTEN CERCA---------------------------
            stations = nodes_stations
            print("----------start nodes----------------")
            #print(nodes)
            prev_group =[]
            for station in stations:
                print(station.name)
                sta_temp_dis = node.get_distance_to(station)
                if sta_temp_dis < 0.30:
                    print("estacion a agrupar")
                    print("distancia")
                    print(sta_temp_dis)
                    prev_group.append(station)
                else:
                    print("estacion no se agrupa")
                    print("distancia")
                    print(sta_temp_dis)
            #print("----------end nodes----------------")
            print(" ------------------------dispositivos a agrupar-------------------------------- ")
            print(prev_group)
            print("--------------------------")
            #print(nodes_stations)
            
            
            #------------------------Ver si el historial es el mismo----------------------------
            if len(prev_group)>1:
                #Funcion para comparar si el historial es el mismo en los posibles dispositivos de un grupo
                print("-------COMPARAR HISTORIAL PARA SACAR GRUPO FINAL-----------")
                print(node.params['position'])
                print("------------------")
                h_0 = node.params['position']
                group=[]
                for dev in prev_group:
                    if h_0 == dev.params['position']:
                        group.append(dev)
                print("---------------grupo final")
                print(group)
                print("----final creacion de grupos")
            
                gm.scan_group(group)
            else:
                print("el grupo es menor a 2")
        else:# si el nodo que esta haciendo el movimiento ya esta agrupado
            leader_name = gm.group_leader_info(id_group)
            if (node.name != leader_name):
                print("*-*-+--_*_*_se extrajo el lider")
                print("id_group: ", str(id_group))
                print("leader: ", str(leader_name))
                stations = nodes_stations
                for station in stations:
                    #print(station.name)
                    if station.name == leader_name:
                        #print("estacion a agrupar")
                        leader = station
                leader_name = str(leader.name)
                node_name = str(node.name)
                leader_position = leader.params['position']
                node_position = node.params['position']
                distance_node_to_leader  = node.get_distance_to(leader)
                print("leader_name: ", str(leader_name))
                print("node_name: ", str(node_name))
                print("position leader: ", str(leader_position))
                print("position node: ", str(node_position))
                print("distancia entre los dos: ", str(distance_node_to_leader))
                if (distance_node_to_leader > 30):
                    print("Eliminar node del grupo")
                    gm.delete_disp(id_group, node)
                else:
                    print("Sigue en el grupo")
            else:
                print("leader_name: ", str(leader_name))
                print("noder_name: ", str(node.name))
                print("node y lider son los mismos")
        gm.groups_info()
        print("----------end nodes----------------")
    
    @classmethod
    def set_pos_original(self, node, pos):
        node.params['position'] = pos
        print("-------------------*********************set_pos_117-----------------------****************")
        if mobility.wmediumd_mode == 3 and mobility.thread_._keep_alive:
            node.set_pos_wmediumd(pos)
    ...
    def check_association(cls, sta, wlan, ap_wlan):
        ...
        print(aps)
        dist = sta.get_distance_to(ap)
        cls.handover2(sta, ap, wlan, ap_wlan)
        cls.ap_in_range(sta, ap, wlan, dist)
        """
        if (str(cls.ac) == 'topsis'):
            for ap in aps:
                dist = sta.get_distance_to(ap)
                #cls.handover(sta, ap, wlan, ap_wlan)
                #cls.handover2(sta, ap, wlan, ap_wlan)
                #cls.handover3(sta, ap, wlan, ap_wlan)
                cls.ap_in_range(sta, ap, wlan, dist)
            if len(aps) > 1:
                print(" ***************************** handover topsis 1")
                #cls.handoverTopsis(sta, aps[len(aps)-1], wlan, ap_wlan)
                value = associationControl(sta, aps[len(aps)-1], wlan, cls.ac, ap_wlan)
            elif len(aps) == 1:
                print(" ***************************** handover topsis 2")
                #cls.handoverTopsis2(sta, aps[len(aps)-1], wlan, ap_wlan)
                Association.associate_infra(sta, aps[len(aps)-1], wlan=wlan, ap_wlan=ap_wlan)
            else:
                print(" ***************************** handover topsis 3")
                #cls.handoverTopsis2(sta, aps[len(aps)-1], wlan, ap_wlan)
                print(aps)
                #Association.associate_infra(sta, aps[len(aps)], wlan=wlan, ap_wlan=ap_wlan)
                #Association.associate_infra(sta, aps[len(aps)-1], wlan=wlan, ap_wlan=ap_wlan)
            ...
            for ap in aps:
                ...
                cls.ap_in_range(sta, ap, wlan, dist)
            if len(aps) > 1:
                print(" ***************************** handover saw 1")
                #cls.handoverTopsis(sta, aps[len(aps)-1], wlan, ap_wlan)
                value = associationControl(sta, aps[len(aps)-1], wlan, cls.ac, ap_wlan)
            elif len(aps) == 1:
                print(" ***************************** handover saw 2")
                Association.associate_infra(sta, aps[len(aps)-1], wlan=wlan, ap_wlan=ap_wlan)
            else:
                print(" ***************************** handover saw 3")
                #cls.handoverTopsis2(sta, aps[len(aps)-1], wlan, ap_wlan)
                print(aps)
                #Association.associate_infra(sta, aps[len(aps)], wlan=wlan, ap_wlan=ap_wlan)
                #Association.associate_infra(sta, aps[len(aps)-1], wlan=wlan, ap_wlan=ap_wlan)
    ...
    @classmethod
    def check_association2(cls, sta, wlan, ap_wlan):
        """check association
    ...
    @classmethod
    def handover3(cls, sta, ap, wlan, ap_wlan):
        """handover
        :param sta: station
        :param ap: access point
        :param wlan: wlan ID"""
        ...
        if cls.ac and sta.params['associatedTo'][wlan] \
            ...
            value = associationControl(sta, ap, wlan, cls.ac, ap_wlan)
            ...
            for wlan in range(len(node.params['wlan'])):
                if node.func[wlan] == 'mesh' or node.func[wlan] == 'adhoc':
                    ...
                    if str(cls.ac) == 'topsis':
                        ...
                        print('configure link 3 TOPSIS---------------------------------------------------')
                    elif str(cls.ac) == 'saw':
                        cls.check_association(node, wlan, ap_wlan=0)
                        print('configure link 3 SAW--------------------------------------------------------')
                    elif str(cls.ac) == 'rf':
                        cls.check_association(node, wlan, ap_wlan=0)
                        print('configure link 3 RF--------------------------------------------------------')
                    elif str(cls.ac) == 'dl':
                        cls.check_association(node, wlan, ap_wlan=0)
                        print('configure link 3 DL--------------------------------------------------------')
                    else:
                        print("****************************************************" + str(cls.ac) + "**********************************************")
                        for ap in cls.aps:
                            for ap_wlan in range(len(ap.params['wlan'])):
                                if ap.func[ap_wlan] != 'mesh' and ap.func[ap_wlan] != 'adhoc':
                                    if cls.wmediumd_mode == 3:
                                        ...
                                        cls.check_association(node, wlan, ap_wlan=ap_wlan)
                                        print('configure link 2')
                    """
                    if (str(cls.ac) != 'topsis') or (str(cls.ac) != 'saw'):
                        
                    else:
                        cls.check_association(node, wlan, ap_wlan=0)
                        print('configure link 3')
                    """
        sleep(0.0001)

    @classmethod
    def configureLinks2(cls, nodes):
        for node in nodes:
            for wlan in range(len(node.params['wlan'])):
                if node.func[wlan] == 'mesh' or node.func[wlan] == 'adhoc':
                    pass
