...
class Node_wifi(Node):
    ...
    def setPosition(self, pos):
        ...
        hist = self.params.get("hist", "null")
        #guardar historial de posiciones
        print("node.py - position to save " + str(self.params['position']))
        del hist[0][0]
        hist[0].append(str(self.params['position']))
    ...
    def setPosition_group(self, pos, net):
        "Set Position"
        self.params['position'] = [float(x) for x in pos.split(',')]
        self.updateGraph()
        ...
        for station in stations:
            print(station.name)
            sta_temp_dis = self.get_distance_to(station)
            if sta_temp_dis < 10:
