...
def myNetwork():
	...
	for data in raw_data:
		...
		sta2.setPosition("{}".format(line[6]))
		sta3.setPosition("{}".format(line[11]))
		...
		for i in range(len(aps)):
			for i2 in range( len(sta1.params["apsInRange"]) ):
				...
				if aps[i] == ap_temp_name:
					...
					ap_temp_ocu = len(ap_temp.params['associatedStations'])
		...
		for i in range( len(sta1.params["apsInRange"]) ):
			...
			f_datos_entrenamiento.write("{},{},{},{},".format(str(ap_temp.name), str(rssi_aptemp), str(n_est_aptemp),str(bat_temp)))
