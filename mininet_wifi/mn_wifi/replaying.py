...
class replayingMobility(object):
    ...
    def notimestamp_(self, node, time_, nodes_stations):
        if time_ >= node.currentTime:
            for n in range(0, node.params['speed']):
                if len(node.position) > 0:
                    ...
                    mobility.set_pos(node, pos, nodes_stations)
                    ...
                    if rss != "null":
                        ...
                        f_stations_info.write('{},{},{},{},{}\n'.format( str(node.name), str(node.params.get('associatedTo')[0]), str(node.params['battery']), str(node.params['rssi']), str(node.params['position']) ) )
                        #f_stations_info.write("{},{},{},{},{},{},{}\n".format("station","aps", "ap", "rssi", "num_sta", "dist", "cons_bat", "associatedTo"))
                        f_stations_info.close()
                    """
                    print("-----------------------------**********************************Cambio pos AQUI-----------------------------**********************************")
                    print(pos)
                    print(node.name)
    ...
    def mobility(self, nodes, Mininet_wifi):
        if nodes is None:
            ...
            nodes_stations = Mininet_wifi.stations
        for node in nodes:
        ...
        while mobility.thread_._keep_alive:
            ...
            for node in nodes:
                if hasattr(node, 'position'):
                    calc_pos(node, time_, nodes_stations)
                    if len(node.position) == 0:
                        nodes.remove(node)
